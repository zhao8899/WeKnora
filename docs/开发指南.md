...
如果你希望通过 GitHub Actions 手动触发同样的检查，可以配置仓库 secrets：
...
- `/health`
  轻量摘要，适合快速确认服务、迁移版本、DocReader、stream manager 是否就绪
- `/system/diagnostics`
  更完整的运行配置快照，适合检查 DB/Redis/docreader/retrieval/object storage 的实际连接与配置摘要
